...
class TravellerModel:
    ...
    def create_traveller(admin_username: str, first_name: str, last_name: str, 
        ...
        try:
            ...
            encrypted_birthday = encrypt_data(birthday)
            ...
            db.execute_non_query(query, (
                encrypted_first_name, encrypted_last_name, encrypted_birthday, encrypted_gender,
                encrypted_street_name, encrypted_house_number, encrypted_zip_code, encrypted_city,
                encrypted_email, encrypted_phone, encrypted_license, encrypted_registration_date
            ))
    ...
    def update_traveller(admin_username: str, traveller_id: int, **updates) -> bool:
        ...
        try:
            ...
            if 'birthday' in updates:
                encrypted_updates['birthday'] = encrypt_data(
                    InputValidator.validate_date(updates['birthday'], "Birthday")
                )
            if 'gender' in updates:
                gender = updates['gender'].lower()
                if gender not in ['male', 'female']:
                    raise ValidationError("Gender must be 'male' or 'female'")
                encrypted_updates['gender'] = encrypt_data(gender)
            ...
            if 'zip_code' in updates:
                encrypted_updates['zip_code'] = encrypt_data(
                    InputValidator.validate_zip_code(updates['zip_code'])
                )
            if 'city' in updates:
                encrypted_updates['city'] = encrypt_data(
                    InputValidator.validate_city(updates['city'])
                )
            if 'email' in updates:
