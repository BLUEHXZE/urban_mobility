...
class LogService:
    ...
    def log_activity(username: Optional[str], description: str, additional_info: str = "", suspicious: bool = False):
        ...
        try:
            ...
            # Encrypt all log data, including date and time
            encrypted_date = encrypt_data(current_date)
            encrypted_time = encrypt_data(current_time)
            ...
            db.execute_non_query(query, (
                encrypted_date, encrypted_time, encrypted_username,
    ...
    def get_logs_for_admin(admin_username: str) -> list:
        ...
        for log in encrypted_logs:
            try:
                decrypted_log = {
                    'id': log[0],
                    'date': decrypt_data(log[1]),
                    'time': decrypt_data(log[2]),
